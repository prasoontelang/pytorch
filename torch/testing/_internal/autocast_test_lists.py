...
class AutocastCPUTestLists:
    ...
    def __init__(self, dev):
        ...
        self.torch_bf16 = [
            ...
            ("conv_transpose3d", conv_args_fp32[2]),
            ("prelu", pointwise0_fp32 + element0_fp32),
            ("_native_multi_head_attention", (torch.randn((n, n, n), device=dev, dtype=torch.bfloat16),
                                              torch.randn((n, n, n), device=dev, dtype=torch.bfloat16),
                                              torch.randn((n, n, n), device=dev, dtype=torch.bfloat16),
                                              n, 4, torch.randn((3 * n, n), device=dev, dtype=torch.bfloat16),
                                              torch.randn((3 * n), device=dev, dtype=torch.bfloat16),
